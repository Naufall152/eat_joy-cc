...
@php $pageTitle='Tambah Resep'; @endphp

@section('content')
<div class="card card-soft">
    <div class="card-header fw-bold"><i class="bi bi-plus-circle"></i> Tambah Resep</div>
    <div class="card-body">
        <form method="POST" action="{{ route('admin.recipes.store') }}">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" name="calories" required value="{{ old('calories') }}">
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" rows="4" name="ingredients" required>{{ old('ingredients') }}</textarea>
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Instructions</label>
                <textarea class="form-control" rows="5" name="instructions" required>{{ old('instructions') }}</textarea>
            </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select" name="premium_plan">
                        <option value="">-</option>
                        <option value="starter" {{ old('premium_plan')==='starter'?'selected':'' }}>starter</option>
                        <option value="starter_plus" {{ old('premium_plan')==='starter_plus'?'selected':'' }}>starter_plus</option>
                    </select>
                </div>
            </div>
